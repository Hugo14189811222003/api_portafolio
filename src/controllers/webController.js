...
const createWeb = async (req, res) => {
    const { id_proyecto, titulo, descripcion, img, link_github, link_demo } = req.body;
    try {
        const { rows } = await pool.query('INSERT INTO web (id_proyecto, titulo, descripcion, img, link_github, link_demo) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id', [id_proyecto, titulo, descripcion, img, link_github, link_demo]);
        res.status(201).json({ id: rows[0].id, id_proyecto, titulo, descripcion, img, link_github, link_demo });
    } catch (error) {
...
const updateWeb = async (req, res) => {
    ...
    const { id_proyecto, titulo, descripcion, img, link_github, link_demo } = req.body;
    try {
        const result = await pool.query('UPDATE web SET id_proyecto = $1, titulo = $2, descripcion = $3, img = $4, link_github = $5, link_demo = $6 WHERE id = $7', [id_proyecto, titulo, descripcion, img, link_github, link_demo, id]);
